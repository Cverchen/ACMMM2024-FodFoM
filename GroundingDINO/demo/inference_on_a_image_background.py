...
def plot_boxes_to_image(flag, image_pil, tgt, per_image_path):
    ...
    for box, label in zip(boxes, labels):
        ...
        if area/image_area <= 0.5:#当检测出来的区域与整体图像区域的占比在0.5以下
...
def plot_boxes_to_image_cifar(flag, image_pil, tgt, image_origin):
    # image_pil_orgin = Image.open(per_image_path).convert("RGB")
    image_pil_origin = np.array(image_origin)
    image_pil_temp = image_pil_origin.copy()
    image_area = image_origin.width * image_origin.height
    H, W = tgt["size"]
    boxes = tgt["boxes"]
    labels = tgt["labels"]
    assert len(boxes) == len(labels), "boxes and labels must have same length"
...
def load_image_cifar(image_pil):
    # load image
    # image_pil = Image.open(image_path).convert("RGB")  # load image
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file", "-c", default='groundingdino/config/GroundingDINO_SwinT_OGC.py',type=str, required=True, help="path to config file")
    parser.add_argument(
        "--checkpoint_path", "-p", type=str, default='weights/groundingdino_swint_ogc.pth',required=True, help="path to checkpoint file"
    )
    parser.add_argument("--image_path", "-i", type=str, required=False, help="path to image file")
    parser.add_argument("--text_prompt", "-t", type=str, required=False, help="text prompt")
    parser.add_argument(
    ...
    with open('datasets/ImageNet100/imagenet100_classindex.txt', 'r', encoding='utf-8') as f:
        lines = f.read().splitlines()
        for line in lines:
            class_index.append(line)
    with open('datasets/ImageNet100/imagenet100_classname.txt', 'r', encoding='utf-8') as f:
        lines = f.read().splitlines()
        for line in lines:
    ...
    # cifar10
    model = load_model(config_file, checkpoint_path, cpu_only=args.cpu_only)
    cifar10 = dset.CIFAR10(root='datasets/cifar10', train=True, transform=trn.Compose([trn.Resize(224), trn.ToTensor()]))
    classname = cifar10.classes
    output_dir = 'CIFAR10_background'
    cifar10_loader = torch.utils.data.DataLoader(
                cifar10, batch_size=1, shuffle=False,
                num_workers=6, pin_memory=False, sampler=None)
    index = 0
    for image, classindex in tqdm(cifar10_loader):
        image_origin = trn.ToPILImage()(image[0])
        text_prompt = classname[classindex]
        os.makedirs(output_dir, exist_ok=True)
        # load image
        image_pil, image = load_image(image_origin)
        # load model
        # set the text_threshold to None if token_spans is set.
        if token_spans is not None:
            text_threshold = None
        ...
        # import ipdb; ipdb.set_trace()
        flag = False
        image_with_box,_,image_clip,flag = plot_boxes_to_image_cifar(flag, image_pil, pred_dict, image_origin)
        # pred_dict 预测的东西
        if flag:
            cv2.imwrite(os.path.join(output_dir,"{}.jpg".format(index)), image_clip)
        index += 1
    
    #cifar100
    model = load_model(config_file, checkpoint_path, cpu_only=args.cpu_only)
    cifar100 = dset.CIFAR100(root='datasets/cifar100', train=True, transform=trn.Compose([trn.Resize(224), trn.ToTensor()]))
    classname = cifar100.classes
    output_dir = 'CIFAR100_background'
    cifar100_loader = torch.utils.data.DataLoader(
                cifar100, batch_size=1, shuffle=False,
                num_workers=6, pin_memory=False, sampler=None)
    index = 0
    for image, classindex in tqdm(cifar100_loader):
        image_origin = trn.ToPILImage()(image[0])
        text_prompt = classname[classindex]
        os.makedirs(output_dir, exist_ok=True)
        # load image
